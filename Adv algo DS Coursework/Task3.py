...
import math
# file handler
#For task 3 we are using Breath first search(BFS) instead of Dijkstras.
...
for _, row in df.iterrows():
    ...
    duration = 1
    '''We used almost similar code from task1a but we changed in order to find the minimum number of stops from A-B we needed to make the edge as 1 to denote 1 hop/jump
    so that The code can work on Dijkstras function instead of Breath first search algorithm'''
    line = row[0]
...
if start_station_index is None or end_station_index is None:
    ...
    d, pi = bfs(tube_graph, start_station_index)
    ...
    if shortest_path_indices:
        ...
        print(f"Minimum number of stops between {start_station_name} and {end_station_name} are: {len(shortest_path_names)-2}")
    else:
...
for start_idx in range(1,num_stations):
    ...
    for end_idx in range(1,num_stations):
        if start_idx != end_idx:  # Avoid calculating stops for the same station
            ...
            stops_list.append(stops-2) # remove start and end station from hops count
